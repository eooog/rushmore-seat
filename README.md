...
# Queue Concurrency: 언제 원자성이 필요하고 언제 필요 없는가

이 문서는 대기열(`application/queue`) 구현 과정에서 반복적으로 부딪힌 "이 Redis 연산 두 개를 원자적으로 묶어야 하나?"라는 질문에 대해, 매번 새로 판단하지 않도록 기준과 실제 사례를 정리한 것이다.

## 핵심 원칙: Redis Lua의 원자성은 롤백이 아니다

Lua 스크립트(`EVAL`)의 원자성이 보장하는 건 딱 하나, **"이 스크립트가 실행되는 동안 다른 어떤 클라이언트의 명령도 끼어들 수 없다"**는 것뿐이다. RDB의 트랜잭션 커밋처럼 "중간에 실패하면 앞서 실행된 것도 되돌린다"는 뜻이 아니다. 스크립트 안에서 3번째 명령이 런타임 에러로 실패해도, 이미 실행된 1·2번째 명령은 그대로 반영된 채 남는다.

메커니즘도 RDB와 다르다. RDB는 MVCC/락으로 "커밋 전까지 다른 트랜잭션에게 안 보이게" 가시성을 미루는 방식이라 동시성은 유지된다. Redis는 단일 스레드 실행 모델이라, 스크립트가 끝날 때까지 **다른 요청 처리 자체를 전면 중단**해서 같은 결과(중간 상태를 아무도 못 봄)를 얻는다. 즉 간극 자체가 없어지는 게 아니라, 그 간극을 외부에서 관측할 수 없게 되는 것이다.

## 판단 기준: 두 가지 질문

새 기능에서 Redis 쓰기가 여러 개 필요할 때, 아래 순서로 판단한다.

**1) 이 쓰기들은 무조건 같이 일어나야 하는가, 아니면 조건부로 하나가 다른 하나를 취소시켜야 하는가?**
후자(조건부 롤백이 필요한 경우)는 이 프로젝트의 큐 구현엔 아직 등장하지 않았다. 전부 "무조건 같이" 케이스였다.

**2) "무조건 같이"인 경우, 그 중간 상태(하나만 반영된 상태)를 외부에서 관측할 수 있는가?**
- 관측 가능하고, 그 관측이 잘못된 응답(오답)으로 이어진다 → Lua로 묶어야 함
- 애초에 재시도로 자연 복구되고 피해가 없다 → 굳이 안 묶어도 됨

## 사례 1 — `enter()`: 묶지 않은 경우

`addWaitingMember`(큐 진입) 하나뿐이라 애초에 여러 쓰기가 아니었다. 과거엔 순번(`joinedAtMillis`)까지 응답에 싣기 위해 `getWaitingRank()` 읽기를 뒤에 붙였다가, "쓰기는 성공했는데 읽기 실패로 전체가 500 나는" 문제가 생겨서 응답 구조를 나눴다(커맨드는 쓰기만 책임지고, 순번 조회는 `GET /queue/me`로 분리). **판단**: 쓰기 자체가 하나뿐이면 원자성 문제가 성립하지 않는다.

## 사례 2 — `admit()`: 묶은 경우

`popWaitingMembers(limit)`로 최대 N명을 한 번에 대기열에서 빼낸 뒤, 그 목록을 순회하며 admission 레코드를 한 명씩 썼다. 이 사이(대기열에서는 빠졌지만 admission 기록은 아직 없는 구간)에 그 회원이 `GET /queue/me`를 호출하면 두 상태 어디에도 안 걸려 404가 났다. 10,000명 규모 부하테스트에서 1.15%(1,700건)로 실제 재현됨.

**판단**: 관측 가능하고, 관측 시 오답(404)으로 이어진다 → Lua로 묶음. `ZPOPMIN` + admission/token/occupancy 3개 쓰기를 회원 1명 단위로 하나의 스크립트에 넣어, pop과 쓰기 사이 간극을 외부에서 관측 불가능하게 만들었다.

## 사례 3 — `getStatus()`: 순서만 바꿔서 해결한 경우

`findAdmissionByMember()`(읽기1)와 `getWaitingRank()`(읽기2)를 순서대로 호출했는데, 그 사이에 `admit()`이 끼어들면 둘 다 null이 되어 404가 났다(사례 2보다 훨씬 좁은 확률이지만 10,000명 규모에서 6건 발생).

**판단**: 이건 Lua로 묶을 필요가 없었다. `admit()`이 이미 원자적이므로 "대기 중"과 "허가됨"은 상호 배타적 상태라는 게 보장된다. 그렇다면 **먼저 대기 여부를 확인**해서 찾으면 바로 반환하고, 못 찾았을 때만 admission을 조회하면, 두 번째 조회 시점엔 admit()이 이미 완전히 끝난 상태라는 게 논리적으로 보장된다. 읽기 순서를 "쓰기가 만든 배타성"에 맞춰 정렬하는 것만으로 경쟁 자체가 성립하지 않게 된다.
...
|---|---|
| 쓰기가 하나뿐 | 아무것도 안 해도 됨 |
| 쓰기 여러 개, 중간 상태 관측 불가/무해 | 그대로 둠 (재시도로 복구) |
| 쓰기 여러 개, 중간 상태 관측 가능 + 오답 유발 | Lua로 하나의 스크립트에 묶음 |
| 읽기 여러 개, 쓰기 쪽이 이미 원자적 | 읽기 순서를 쓰기가 보장하는 배타성에 맞춰 정렬 |