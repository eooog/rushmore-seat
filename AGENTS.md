...
`rushmore-seat` is a large-scale seat reservation system simulation. The goal is not basic reservation CRUD. The system must preserve the following architectural goals:
...
Agents working on this repository must treat the README architecture as the source of truth unless the task explicitly asks to change that architecture.
...
### 1. Preserve the Domain Separation
...
* Do not add time-overlap constraints casually. If needed, implement as explicit service validation or PostgreSQL exclusion constraint after design review.
...
The queue should absorb large traffic. Only admitted users should reach the seat selection / hold path.
...
* Confirm must verify that the hold token is valid and `hold_expires_at > now()`.
...
Kotlin package path should remain stable unless the task explicitly asks for a broad package rename.
...
* Do not hide important domain rules inside framework magic.
...
Do not claim a feature is complete unless the relevant tests have been added or updated and the verification command has been run.
...
Do not state that a change is verified if the verification command was not run.
...
2. Identify the affected architecture area:
    ...
    * WebSocket backplane
    * frontend rendering
3. Find existing tests for that area.
4. Decide whether the change affects correctness, performance, schema, or API shape.
...
* Rename broad packages casually.
* Replace the architecture with a simpler CRUD model.
* Introduce full-seat broadcast.
* Use queue admission as reservation ownership.
* Use Redis as the final source of truth for reservation.
* Add persistent seat states without recovery semantics.
* Hold DB transactions across user wait time.
* Make performance claims without measurement.
* Modify schema without Flyway migration.
* Leave failing or missing tests unexplained.
...
* Do not log admission tokens, hold tokens, or user-sensitive identifiers in production-style logs.
...
* It runs the relevant verification command.
* It reports any unverified command honestly.
